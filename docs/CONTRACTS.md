# Contracts

> Purpose: 记录当前生效的数据契约、模板骨架与变更联动规则。
> Status: active
> Scope: 数据模型职责、关键字段语义、模板化结构、变更影响矩阵
> Out of scope: 视觉样式、阶段进度、具体录入命令教程
> Update triggers: 数据结构变化、模板变化、workflow 联动关系变化
> Priority: 3

## 内容目录单元

以电影为例，当前单条内容目录包含：

```text
content/video/movie/{ID}/
  data.json
  source.json
  index.md
  raw/
  images/
```

各文件职责：

- `data.json`
  - 前台正式结构化数据来源
- `source.json`
  - 字段来源与溯源记录
- `index.md`
  - 审阅与归档材料，不直接进入前台展示链路
- `raw/`
  - 原始抓取材料、对比报告、过程产物
- 图片目录
  - 海报、剧照、头像、视频缩略图等本地资源

## ID 与本地工坊职责

### 内容 ID 规则

当前内容 ID 采用：

```text
MMSSNNNNNN
```

说明：

- `MM` 表示一级模块编号
- `SS` 表示子模块编号
- `NNNNNN` 表示该子模块下的递增序号
- `id` 是稳定主标识，标题变化不影响 `id` 与详情路由

### 本地工坊与 SQLite 职责

SQLite 只用于本地内容工坊，不作为线上数据库。

当前稳定职责包括：

- 导入任务记录
- 待确认条目记录
- 候选匹配缓存
- 来源数据缓存
- 图片与资源记录
- ID 分配记录

## 当前关键字段语义

### `synopsis`

- 用于列表页、首页卡片和详情页顶部短简介
- 不是完整剧情

### `story`

- 用于详情页 `详情介绍` Tab 的完整剧情
- 已上映作品：应能独立阅读，覆盖主要人物、关键转折与结局
- 未上映 / 未公开完整剧情作品：只能整理公开剧情物料，不得补写未公开后续
- 这类条目的 `story.note` 必须明确标注“基于公开剧情物料整理，非完整剧情/非完整人生全程”

### `reviews`

- 只保留精选长评或高质量评语
- 不再用一句话短评或热门短评直接充数
- 来源优先使用豆瓣长评页，并允许人工筛选整理

### `images.poster` 与 `images.posters`

- `images.poster` 永远表示主海报
- `images.posters` 表示补充海报列表，不包含主海报
- `images.postersTotal` 是源站可获取总量元数据，不等于本地数组长度

### `images.stillsTotal`

- 表示源站可获取剧照总量，不等于当前本地已下载文件数量

### `similar`

- 当前采用最小统一方案
- 已收录站内作品可补 `id`
- 未收录作品保留摘要结构：`title` / `year` / `rating`

### `genre` 与 `tags`

- `genre` 与 `tags` 是两套不同语义，不得混用
- `genre` 表示外部平台相对标准化的类型分类
- `tags` 同时包含外部平台标签与手动维护标签
- 列表页筛选 UI 中不区分标签来源，统一作为一个标签集合供用户筛选

### `links`

- 外部来源链接统一放在 `links`
- 缺失值统一使用 `null`

## 当前稳定模板骨架

### 电影录入模板

电影条目当前必须维护：

- `data.json`
- `source.json`
- `index.md`
- 必要图片资源

执行细则继续以 `.opencode/skills/movie-entry-workflow/` 下文档为准。

### 电影详情页共用骨架

当前电影详情页的稳定骨架为：

- 顶部基础信息区
- 下方 Tab 分区：
  - 详情介绍
  - 演职员
  - 精彩影评
  - 视频
  - 图片
  - 音乐
  - 外部来源
- 右侧栏：
  - 系列作品
  - 相似作品
- 底部评论区（Giscus）

详情页共用展示约束：

- 顶部只展示一部分最重要的信息，不展示全量资料
- 顶部主演只展示前几位，并以文字方式呈现
- 完整人物展示下沉到 `演职员` Tab
- `图片` Tab 使用统一图片流，不分成两个大分区单独展示“海报 / 剧照”
- `外部来源` Tab 使用简洁链接列表，只展示站点名与跳转链接
- `视频` Tab 只展示封面、标题、来源并跳转外部页面，不做站内播放器

### 影视列表页共用骨架

- 页面标题区
- 搜索与筛选区
- 内容卡片区
- 分页区

当前列表页支持两种阅读方向：

- 网格模式：更适合首页预览与快速扫视
- 列表模式：更适合影视模块主列表与高信息密度浏览

补充约束：

- 两种布局都应支持深色 / 浅色主题
- 布局切换不改变筛选结果本身
- 首页模块预览固定使用网格卡片，不单独设计另一套首页专用卡片体系

## 变更影响矩阵

### 当 `data.json` 字段语义或结构变化时

必须同步检查并更新：

- `CONTRACTS.md`
- `.opencode/skills/movie-entry-workflow/SKILL.md`
- `FIELD-SOURCE-MAPPING.md`
- `FIELD-VALIDATION.md`
- `DATA-TO-MD-MAPPING.md`
- `INDEX-MD-TEMPLATE.md`
- 对应真实条目的 `source.json`
- 未来前台读取逻辑

### 当详情页 Tab 结构变化时

必须同步检查并更新：

- `UI-GUIDE.md`
- `CONTRACTS.md`
- `PROJECT.md`（如果影响 V1 范围或主共识）
- `index.md` 审阅模板和映射文档（如受影响）

### 当列表页卡片骨架变化时

必须同步检查并更新：

- `UI-GUIDE.md`
- `CONTRACTS.md`
- 首页模块预览卡片相关说明

### 当录入 workflow 规则变化时

必须同步检查并更新：

- `.opencode/skills/movie-entry-workflow/` 下执行细则
- 本文档中的契约摘要
- `STATUS.md` 中最近关键变更（如会影响当前阶段判断）

## 与 workflow 文档的边界

当前约定如下：

- `docs/CONTRACTS.md` 负责讲“当前项目采用什么数据与模板契约”
- `.opencode/skills/movie-entry-workflow/` 负责讲“电影具体如何录入、如何校验、如何生成产物”

如果只是电影录入执行细节变化，不需要把全部细节复制进 `docs/`。

## 当前约束总结

- 不推翻当前 workflow 重来
- 以“承认现有事实标准，再局部统一”为原则
- 模板变化必须配套更新变更影响范围
