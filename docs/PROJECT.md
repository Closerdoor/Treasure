...
- 后台采用原生 Node 服务与定制前端，不使用 Directus，优先围绕作品人工校正流程设计界面。
- 直接读写 `.local/treasure.db`，用于人工增删改查作品、人物关系、分类关系和结构化 JSON 字段。
- 当前覆盖影视作品列表、搜索、状态筛选、分页、新增、删除、基础字段编辑、结构化 JSON 字段编辑、演职员关系维护、分类 / 标签关系维护、人物检索添加，以及书籍基础字段与 JSON 字段维护。
- 界面按作品维护台组织，保留海报、标题、评分、演职员和分类等上下文；影视作品编辑页采用左侧表单、右侧前台预览结构，不把人工校正体验降级成纯字段表格。
- 字段表单同时展示数据库字段名、中文说明和前台用途，字段说明配置位于 `admin/public/field-meta.js`。
- 影视作品保存基础信息、结构化 JSON、演职员关系或分类关系后，会自动执行 `node tools/db/export-generated.mjs`，使本地 Astro 前台刷新后读取最新 generated 数据。线上发布仍然使用静态构建。
- 启动时在 `.local/backup/` 下生成一次数据库备份。

边界：
- 不参与 Astro 构建。
- 不直接读取 `generated/`。
- 不直接改写 `generated/` 或 `site/public/assets/`；影视作品保存后只通过 `tools/db/export-generated.mjs` 统一刷新静态数据和资源。
- 不替代采集脚本、导出脚本或发布流程。
- 暂不提供字段级审计日志；后台 API 修改不会记录 before / after 历史。若未来需要追溯人工改动，应新增本地审计表并只记录后台 API 写入行为。
