...
- `content/` 是历史内容目录或人工审阅材料，目前不是前台正式数据源。
...
- 收藏馆只收录经过馆长筛选的作品，不追求全量覆盖。
- 作品资料以公开社区信息、标准元数据和人工确认结果为主。
- 原始抓取材料、候选结果和审阅材料应留在本地工作区或实验目录，不直接进入前台。
- 进入 `.local/treasure.db` 的数据应是已确认的结构化主数据。
- 进入 `generated/` 的数据应是前台可直接消费的稳定投影。
...
当前前台不是传统影视资源站，也不是博客，而是“资料馆气质 + 现代浏览体验”的静态收藏站。

原则：

- 深色资料馆风格，突出作品封面、海报和结构化信息。
- 首页负责说明收藏馆气质和模块入口，不做长篇博客式叙述。
- 列表页优先支持浏览、筛选、分页和卡片 / 列表视图切换。
- 详情页优先展示作品基础信息、评分、简介、演职员、图片、评论和外部链接。
