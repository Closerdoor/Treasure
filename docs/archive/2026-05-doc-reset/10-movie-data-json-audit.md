...
- 现已统一规则：
  - `postersTotal` 表示源站可获取的补充海报总量，不等于本地已下载文件数量
  - `stillsTotal` 表示源站可获取的剧照总量，不等于本地已下载文件数量
- 这两个字段应视为总量元数据，而不是当前数组长度
...
- 当前已采用最小统一方案：
  - 对于已收录在站内的作品，可以补 `id`
  - 未收录作品继续保留 `title` / `year` / `rating` 摘要结构
- 这样可以在不破坏现有数据的前提下，逐步支持详情页右栏稳定跳转
