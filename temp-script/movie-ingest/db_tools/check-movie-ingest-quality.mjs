...
function normalizeMode(value) {
  return value === 'staging' ? 'staging' : 'staging';
}

function rootsForMode(mode) {
  return {
    movieRoot: PATHS.stagingDir,
...
async function collectIds(args, movieRoot) {
  ...
  throw new Error('Usage: node db_tools/check-movie-ingest-quality.mjs --ids <id1,id2> [--mode staging] [--strict]');
}
