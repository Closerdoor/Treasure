...
async function main() {
  ...
  if (!args.input) {
    console.log('Usage: node tools/db/run-movie-batch-workflow.mjs --input <tasks.json> [--output-mode staging|new-flow] [--full-pipeline] [--allow-skipped] [--enforce-high-standard]');
    return;
  }

  const outputMode = args['output-mode'] === 'new-flow' ? 'new-flow' : 'staging';
  const enforceHighStandard = Boolean(args['enforce-high-standard']);
  const intakeScript = path.join('tools', 'db', 'run-movie-intake-from-tasks.mjs');
  const qualityScript = path.join('tools', 'db', 'check-movie-ingest-quality.mjs');
  ...
  if (args['full-pipeline']) {
    if (outputMode !== 'staging') {
      throw new Error('--full-pipeline requires --output-mode staging');
    }

    const importStep = runNode(path.join('tools', 'db', 'import-movies.mjs'), []);
    assertOk('import', importStep);
    pipeline.push({ step: 'import', ok: true, stdout: importStep.stdout });

    const exportStep = runNode(path.join('tools', 'db', 'export-generated.mjs'), []);
    assertOk('export', exportStep);
    pipeline.push({ step: 'export', ok: true, stdout: exportStep.stdout });

    const assetsStep = runNode(path.join('tools', 'db', 'check-assets.mjs'), []);
