...
const outputRoot = path.join(repoRoot, '.local', 'new-flow', 'video', 'movie');
const sourceRoot = path.join(repoRoot, '.local', 'new-flow-field-sources', 'video', 'movie');
const legacySampleRoot = path.join(repoRoot, '.local', 'new-flow', 'video', 'movie');
const legacySourceRoot = path.join(repoRoot, '.local', 'new-flow-field-sources', 'video', 'movie');
...
function resolveOutputRoots(mode) {
  if (mode === 'staging') {
    return {
      outputRoot: path.join(repoRoot, '.local', 'staging', 'video', 'movie'),
...
async function main() {
  ...
  if (!args.input) {
    console.log('Usage: node tools/db/run-movie-intake-from-tasks.mjs --input <tasks.json> [--output-mode new-flow|staging]');
    return;
  }
