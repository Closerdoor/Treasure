...
async function main() {
  ...
  lines.push('');
  lines.push('> 本文档用于验收当前 4 条电影样板在“旧数据 -> 新流程数据 -> 数据库目标字段”这一层是否对齐。');
  lines.push('> 旧的当前数据内容：来自 `.local/staging/video/movie/*.json`。');
  lines.push('> 新流程数据内容：来自 `.local/new-flow/video/movie/*.json`。');
  lines.push('> 数据来源或处理逻辑：来自 `.local/new-flow-field-sources/video/movie/*.json`，并补充必要的派生说明。');
  lines.push('');
