...
const legacyMovieRoot = path.join(repoRoot, '.local', 'staging', 'video', 'movie');
const legacySourceRoot = path.join(repoRoot, '.local', 'field-sources', 'video', 'movie');
const newMovieRoot = path.join(repoRoot, '.local', 'new-flow', 'video', 'movie');
const newSourceRoot = path.join(repoRoot, '.local', 'new-flow-field-sources', 'video', 'movie');
...
function buildReviewMetadataByMovieId(movieId) {
  ...
  if (movieId === '0101000003') {
    return [
      { title: '阿甘的爱情', url: 'https://movie.douban.com/review/1436379/' },
      { title: '飘飞的羽毛', url: 'https://movie.douban.com/review/1000747/' },
      { title: '一羽人生', url: 'https://movie.douban.com/review/1012226/' },
  ...
  if (movieId === '0101000002') {
    return [
      ...
      { title: '老粉的失望', url: 'https://movie.douban.com/review/17567438/' },
      { title: '值得去影院一听，但作为电影配不上流行音乐之王的地位', url: 'https://movie.douban.com/review/17566820/' },
...
async function main() {
  ...
  for (const fileName of files) {
    ...
    if (movie.id === '0101000002') {
      ...
      newSources.publishCompany = makeSystemEntry(newMovie.publishCompany, '当前样板缺少稳定出品公司来源，先保留空值');
    }
    newSources.tags = makeSystemEntry(newMovie.tags, '当前4条样板尚未建立标签体系，先保留空数组');
    newSources.series = makeSystemEntry(newMovie.series, '当前4条样板未录入系列关系，先保留空数组');
    newSources.quotes = makeSystemEntry(newMovie.quotes, '当前4条样板未整理 quotes，先保留空数组');
    newSources.tmdbId = tmdbId
      ? makeDerivedEntry(tmdbId, newMovie.links?.tmdb ?? null, '从 links.tmdb URL 解析出 TMDB movie id')
      : makeSystemEntry(null, '当前样板未提供可解析的 TMDB id');
    ...
    if (newSources.reviews?.source === 'douban') {
      newSources.reviews.note = `${newSources.reviews.note || '豆瓣影评页长评'}；已补 review.url/title，全文保留，rating 不再进入数据库`;
    }

    if (newSources.country) {
      newSources.country.value = newMovie.country;
      newSources.country.note = '地区按最早公映地区推断，仅保留单一地区值；电影节与首映场次不作为首发地区依据';
    }

    if (newSources.soundtrack) {
      newSources.soundtrack.note = `${newSources.soundtrack.note || '原声带信息'}；已重组为 albums[] 结构`;
    }
