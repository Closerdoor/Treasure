...
> 兼容说明：这是旧 file-first workflow 的映射文档，仅保留作历史样本兼容参考。
> 当前主流程已切换到 SQLite 主源，因此本文不再定义当前电影录入的最终目标结构。
...
| data.json 字段 | 映射规则 |
|----------------|----------|
| `story.text` | 直接展示（保留换行） |
| `story.note` | 如有数据，追加在末尾 |
