...
> 当前说明：本文档保留为 **字段选源规则** 的有效参考。
> 这里出现的字段名主要对应抓取阶段或规范化中间对象，不等于数据库最终列名。
> 最终落库结构以 `docs/DATABASE.md` 为准，需再映射到 `works` / `people` / `work_credits` / `terms` / `work_terms`。
...
- 该字段的溯源记录必须说明是“完整剧情整理”还是“基于公开剧情物料整理”；当前可记录在导入摘要、临时溯源 JSON 或后续专用附表中。
...
2. **记录冲突**：在导入摘要、临时溯源 JSON 或其它中间承载中记录其他数据源的值
